...
#endif
#include <iostream>
class MainMem_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    MainMem_api_t(VMainMem* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_ld)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_str)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_DataIn)));
        sim_data.inputs.push_back(new VerilatorSData(&(dut->io_Address)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_DataOut)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["MainMem.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    VMainMem* top = new VMainMem;
    std::string vcdfile = "test_run_dir/examples/MainMem/MainMem.vcd";
