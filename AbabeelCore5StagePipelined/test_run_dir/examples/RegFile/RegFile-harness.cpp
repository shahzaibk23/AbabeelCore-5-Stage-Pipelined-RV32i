...
#endif
#include <iostream>
class RegFile_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    RegFile_api_t(VRegFile* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_WriteData)));
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_RegWrite)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_rd2)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_rd1)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["RegFile.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    std::string vcdfile = "test_run_dir/examples/RegFile/RegFile.vcd";
