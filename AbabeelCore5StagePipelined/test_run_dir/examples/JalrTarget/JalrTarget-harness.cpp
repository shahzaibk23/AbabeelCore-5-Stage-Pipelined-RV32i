...
#endif
#include <iostream>
class JalrTarget_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    JalrTarget_api_t(VJalrTarget* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_output)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["JalrTarget.reset"] = 0;
    }
#if VM_TRACE
...
static JalrTarget_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VJalrTarget* top = new VJalrTarget;
    std::string vcdfile = "test_run_dir/examples/JalrTarget/JalrTarget.vcd";
