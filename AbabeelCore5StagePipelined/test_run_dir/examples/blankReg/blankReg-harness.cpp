...
#endif
#include <iostream>
class blankReg_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    blankReg_api_t(VblankReg* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_ins_in)));
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_pc_in)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_ins_out)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_pc_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["blankReg.reset"] = 0;
    }
#if VM_TRACE
...
static blankReg_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    VblankReg* top = new VblankReg;
    std::string vcdfile = "test_run_dir/examples/blankReg/blankReg.vcd";
